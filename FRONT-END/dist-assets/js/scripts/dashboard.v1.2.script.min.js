...
// Relação de projectos por sector
  ...
  $(document).on("change click",".search",function () {
    ...
    updateDashboard2(searchType)
    listarPorProvincia(searchType)
  })/* Fim da pesquisa de dados */


  listarPorProvincia()
  function listarPorProvincia(searchType) {
      ...
      // alert(pesoe+" "+pesoe_text)
      $.ajax({
          url:"./ajax/dashboard/projectoPorProvincia2.php",
          method:"POST",
          data:{
            'sector_id':sector_id,
  ...
  updateDashboard2()
  function updateDashboard2(searchType) {
    ...
    // alert(provincia_id)
    $.ajax({
      url: './app/api/reports/dashboardQuantities2.php',
      type: 'GET',
      dataType: "json",
      data: {
        sector: sector,
      ...
      success: function(response){
        if(sector != "" && instituicao != ""){
          $(".sect_inst").html("<b>"+sector_option+"</b> Instituição: <b>"+instituicao_option+"</b>");
        }else if(instituicao != ""){
        ...
        // $("#totalProjectos").text(response.totalProjects)
        // $("#projectosTerminados").text(response.projectosTerminados)
        // $("#projectosEmCurso").text(response.projectosEmCurso)
        // $("#projectosAtrasados").text(response.projectosAtrasados)
        console.log(response);
        $("#projectosSemExecucao").text(response.projectosSemExecucao)
        $("#projectosEmMobilizacao").text(response.projectosEmMobilizacao)
        $("#projectosEmContratacao").text(response.projectosEmContratacao)
        $("#projectosNaoIniciados").text(response.projectosNaoIniciados)
